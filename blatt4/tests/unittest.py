...
def output_results(results, result_file):
    max_points = 0
    earned_points = 0
    for t in results:
    ...
    if did_skip:
        ...
        if relative_points >= PRESENT_OWN_THRESHOLD:
            print(" ↳ Congratulations, you can present your own solution!")
            result = "own"
        elif relative_points >= PRESENT_ML_THRESHOLD:
            print(" ↳ You must present our solution (musterlösung)!")
            result = "ml"
...
def print_json(results, target):
    total = {
        "tests": results,
        "success": all(walk(results, lambda t: not t.failed and not t.skipped)),
        "points": sum(int(x) for x in walk(results, lambda t: t.points)),
    }
...
def main():
    ...
    if args.json:
        print_json(results, args.json)
    output_results(results, args.result)
